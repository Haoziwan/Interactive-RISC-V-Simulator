...
export function LabelNode({ data, id, selected }: { data: LabelNodeData; id: string; selected?: boolean }) {
  ...
  useEffect(() => {
    ...
    console.log('输入边信息:', inputEdge);

    if (!inputEdge) {
      // 如果没有输入连接，设置默认值0
      console.log('没有输入连接，设置默认值0');
      handleValueChange(0);
      return;
    }
    // 找到源节点
    ...
    if (sourceNode?.data && typeof sourceNode.data === 'object') {
      ...
      console.log('端口ID:', portId);
      console.log('源节点数据结构:', sourceNode.data);
